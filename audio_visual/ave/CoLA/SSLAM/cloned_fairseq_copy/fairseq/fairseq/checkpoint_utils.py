...
def load_model_ensemble_and_task_modified(
    filenames,
    arg_overrides: Optional[Dict[str, Any]] = None,
    task=None,
    strict=True,
        ...
        for shard_idx in range(num_shards):
            ...
            if "fsdp_metadata" in state and num_shards > 1:
                ...
                if "from_checkpoint" in argspec.args:
                    ...
                    # setup lora in config
                    cfg.model.pretrained_model_args.model.use_lora = use_lora
                    cfg.model.pretrained_model_args.model.lora_r = lora_r
                    cfg.model.pretrained_model_args.model.lora_alpha = lora_alpha

                    cfg.model.pretrained_model_args.model.lora_fusion_layers = lora_fusion_layers
                    cfg.model.pretrained_model_args.model.lora_c_dim = lora_c_dim
                    cfg.model.pretrained_model_args.model.lora_reduction = lora_reduction
                    cfg.model.pretrained_model_args.model.lora_c_scaling = lora_c_scaling
               
                    # print(cfg.model.pretrained_model_args.model)
                    model = task.build_model(cfg.model, from_checkpoint=True)
                ...
                model.load_state_dict(
                    state["model"], strict=strict, model_cfg=cfg.model
                    # state["model"], strict=False, model_cfg=cfg.model # For LoRA
                )
