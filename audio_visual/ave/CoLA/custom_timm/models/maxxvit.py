...
from custom_timm.layers import Mlp, ConvMlp, DropPath, LayerNorm, ClassifierHead, NormMlpClassifierHead
from custom_timm.layers import create_attn, get_act_layer, get_norm_layer, get_norm_act_layer, create_conv2d, create_pool2d
from custom_timm.layers import trunc_normal_tf_, to_2tuple, extend_tuple, make_divisible, _assert
from custom_timm.layers import RelPosMlp, RelPosBias, RelPosBiasTf, use_fused_attn, resize_rel_pos_bias_table
...
class AttentionCl(nn.Module):
    ...
    def __init__(
        ...
        if use_lora:
            ...
            self.qkv = lora.MergedLinear(dim, dim_attn * 3, bias=bias, lora_alpha=lora_alpha, r=lora_r, enable_lora=[True, True, True])
        ...
        self.attn_drop = nn.Dropout(attn_drop)
        if use_lora:
            self.proj = lora.Linear(dim_attn, dim_out, bias=bias, lora_alpha=lora_alpha, r=lora_r)
        else:
...
class MaxxVitBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_lora:
            attn_kwargs = dict(dim=dim_out, cfg=transformer_cfg, drop_path=drop_path, use_lora=use_lora,lora_alpha=lora_alpha, lora_r=lora_r)
        else:
            attn_kwargs = dict(dim=dim_out, cfg=transformer_cfg, drop_path=drop_path)
        partition_layer = PartitionAttention2d if self.nchw_attn else PartitionAttentionCl #  Use PartitionAttentionCl
...
class MaxxVitStage(nn.Module):
    def __init__(
        ...
        for i, t in enumerate(block_types):
            ...
            assert t == 'M' ## only support MaxxVitBlock for now
            if t == 'C':
                conv_cls = ConvNeXtBlock if conv_cfg.block_type == 'convnext' else MbConvBlock
                blocks += [conv_cls(
                    in_chs,
...
def _overlay_kwargs(cfg: MaxxVitCfg, **kwargs):
    ...
    for k, v in kwargs.items():
        if k.startswith('transformer_'):
            ...
            base_kwargs[k] = v
    print(cfg)
    print(base_kwargs, transformer_kwargs, conv_kwargs)
    cfg = replace(
        cfg,
...
def get_module_by_path(model: nn.Module, path: str):
    """Mimic get_submodule() for PyTorch <1.9."""
    parts = path.split('.')
    m = model
    for p in parts:
        m = getattr(m, p)
...
def checkpoint_filter_fn(state_dict, model: nn.Module):
    ...
    for k, v in state_dict.items():
        if k.endswith('relative_position_bias_table'):
            path = k[:-29]  # remove ".relative_position_bias_table"
            try:
                m = get_module_by_path(model, path)
                if v.shape != m.relative_position_bias_table.shape or m.window_size[0] != m.window_size[1]:
                    v = resize_rel_pos_bias_table(
                        v,
                        new_window_size=m.window_size,
                ...
                print(f"Warning: failed to access module {path} for resizing. Skipping resize.")

        # adapt between conv2d / linear layers if shape differs but numel matches
        if k in model_state_dict and v.ndim != model_state_dict[k].ndim and v.numel() == model_state_dict[k].numel():
            assert v.ndim in (2, 4)
            v = v.reshape(model_state_dict[k].shape)
...
def _create_maxxvit(variant, cfg_variant=None, pretrained=False, **kwargs):
    ...
    return build_model_with_cfg(
        ...
        pretrained_strict=False, # added for lora support
        **kwargs)
