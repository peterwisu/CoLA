...
from custom_timm.layers import PatchEmbed, Mlp, DropPath, trunc_normal_, lecun_normal_, resample_patch_embed, \
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if use_lora:
            if lora_fusion:
                self.qkv = lora.CMMergedLinear(dim, dim* 3, bias=qkv_bias, enable_lora=[True, True, True], lora_alpha=lora_alpha, r=lora_r, c_features=lora_c_dim, reduction_ratio=lora_reduction, lora_c_scaling=lora_c_scaling)
            else:
                self.qkv = lora.MergedLinear(dim, dim* 3, bias=qkv_bias, enable_lora=[True, True, True], lora_alpha=lora_alpha, r=lora_r)
        else:
        ...
        if use_lora:
            if lora_fusion:
                self.proj = lora.CMLinear(dim, dim,  lora_alpha=lora_alpha, r=lora_r,  c_features=lora_c_dim, reduction_ratio=lora_reduction, lora_c_scaling=lora_c_scaling)
            else:
                self.proj = lora.Linear(dim, dim,  lora_alpha=lora_alpha, r=lora_r)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            # added for lora 
            use_lora=lora_config['use_lora'],
            lora_r=lora_config['lora_r'],
            lora_alpha=lora_config['lora_alpha'],
            lora_c_dim= lora_config['lora_c_dim'],
            lora_reduction=lora_config['lora_reduction'],
            lora_c_scaling=lora_config['lora_c_scaling'],
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            **kwargs,               # Strongly recommended to add this
    ):
        """
        Args:
        ...
        lora_config = {}
        if 'use_lora' in kwargs:
            lora_config['use_lora'] = kwargs.pop('use_lora')
        if 'lora_r' in kwargs:
            lora_config['lora_r'] = kwargs.pop('lora_r')
        if 'lora_alpha' in kwargs:
            lora_config['lora_alpha'] = kwargs.pop('lora_alpha')
        if 'lora_c_dim' in kwargs:
            lora_config['lora_c_dim'] = kwargs.pop('lora_c_dim')
        if 'lora_reduction' in kwargs:
            lora_config['lora_reduction'] = kwargs.pop('lora_reduction')
        if 'lora_c_scaling'  in kwargs:
            lora_config['lora_c_scaling'] = kwargs.pop('lora_c_scaling')

        lora_fusion_layers = kwargs['lora_fusion_layers']

        if len(lora_fusion_layers) != depth:
            raise ValueError("Lora Fusion layer not equal to depth")
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                lora_fusion=lora_fusion_layers[i]
