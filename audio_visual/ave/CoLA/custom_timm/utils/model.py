...
from custom_timm.layers import BatchNormAct2d, SyncBatchNormAct, FrozenBatchNormAct2d,\
...
def _freeze_unfreeze(root_module, submodules=[], include_bn_running_stats=True, mode='freeze'):
    ...
    if isinstance(root_module, (
        ...
        raise AssertionError(
            ...
            "`custom_timm.utils.model.freeze_batch_norm_2d` or `custom_timm.utils.model.unfreeze_batch_norm_2d` instead.")
...
def freeze(root_module, submodules=[], include_bn_running_stats=True):
    ...
    Hint: If you want to freeze batch norm ONLY, use `custom_timm.utils.model.freeze_batch_norm_2d`.

    Examples::

        >>> model = custom_timm.create_model('resnet18')
