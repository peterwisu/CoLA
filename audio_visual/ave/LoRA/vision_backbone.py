...
def get_timm_EVA_pretrained_model(n_classes=527, imgnet=True, use_lora=False, lora_r=0, lora_alpha=0):
    print(f"\n loading EVA with imgnet pretrained {imgnet}\n")
    kwargs = {"use_lora": use_lora ,"lora_r": lora_r, "lora_alpha": lora_alpha} 
...
def get_timm_dino_l_pretrained_model(n_classes=527, imgnet=True, use_lora=False, lora_r=0, lora_alpha=0):
    print(f"\n loading DINOv2 with imgnet pretrained {imgnet}\n")
    kwargs = {"use_lora": use_lora ,"lora_r": lora_r, "lora_alpha": lora_alpha} 
    m = custom_timm.create_model(model_name='vit_large_patch14_dinov2.lvd142m', pretrained=imgnet, pretrained_strict=False ,global_pool='', class_token=True,dynamic_img_size=True,**kwargs)
    return m

def get_timm_dino_b_pretrained_model(n_classes=527, imgnet=True, use_lora=False, lora_r=0, lora_alpha=0):
    print(f"\n loading DINOv2 with imgnet pretrained {imgnet}\n")
    kwargs = {"use_lora": use_lora ,"lora_r": lora_r, "lora_alpha": lora_alpha} 
...
def get_timm_vit_b_pretrained_model(n_classes=527, imgnet=True, use_lora=False, lora_r=0, lora_alpha=0):
    print(f"\n loading DINOv2 with imgnet pretrained {imgnet}\n")
    kwargs = {"use_lora": use_lora ,"lora_r": lora_r, "lora_alpha": lora_alpha} 
