...
def get_parser():
    ...
    parser.add_argument('--granularity', type=str, choices=['all', 'frame', 'utterance'], required=True, 
                        help='Specifies the granularity of features to use: "all" for all frame/patch toekens/features including the cls token, \
                        "frame" for all frame/patch tokens/features excluding the cls token, and "utterance" for only the cls token/feature.')
...
def main():
    ...
    with torch.no_grad():
        try:
            ...
            if granularity == 'all': ## all patch tokens + cls token
            ...
            elif granularity == 'frame': ## all patch tokens except cls token
                feats = model.extract_features(source, padding_mask=None,mask=False, remove_extra_tokens=True)
                feats = feats['x'].squeeze(0).cpu().numpy()
            elif granularity == 'utterance': ## only cls token
                feats = model.extract_features(source, padding_mask=None,mask=False, remove_extra_tokens=False)
                feats = feats['x']
