...
class Data2VecMultiConfig(FairseqDataclass):
    ...
    use_lora: bool = field(
        default=False,
        metadata={"help": "Enable LoRA fine-tuning. Pass --use_lora to activate."}
    )
    lora_r: int = field(
        default=0,
    ...
    lora_c_dim: List[int] = field(
        default_factory=list,  # Empty list
        metadata={"help": "MMLoRA cross features dimesion"}
    )
    lora_reduction: List[int]  = field(
        default_factory=list,  # Empty list
        metadata={"help": "MMLoRa reduction rate"}
    )
    lora_c_scaling: float = field(
        default=0,
        metadata={"help": "MMLoRa reduction rate"}
    )
    lora_fusion_layers: List[bool] = field(
        default_factory=lambda: [False] * 12,
        metadata={"help": "Fusion pattern: [False,True,False,True...] for 12 layers"}
    )
...
class Data2VecMultiModel(BaseFairseqModel):
    ...
    def __init__(self, cfg: Data2VecMultiConfig, modalities, skip_ema=False, task=None):
        ...
        if len(cfg.lora_fusion_layers) !=  cfg.depth:
            raise ValueError("The len of fusion layer does not match sslam depth")
        self.blocks = nn.ModuleList([make_block(dpr[i], 
                                            use_lora=cfg.use_lora,
                                            lora_r=cfg.lora_r,
                                            lora_alpha=cfg.lora_alpha,
                                            lora_fusion=cfg.lora_fusion_layers[i],
                                            lora_c_dim=cfg.lora_c_dim[i],
                                            lora_reduction=cfg.lora_reduction[i],
                                            lora_c_scaling=cfg.lora_c_scaling
