...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        if use_lora:
            import loralib as lora
            if lora_fusion:
                linear_layer = partial(lora.CMLinear, lora_alpha=lora_alpha, r=lora_r,  c_features=lora_c_dim, reduction_ratio=lora_reduction,lora_c_scaling=lora_c_scaling)
                # linear_layer = partial(lora.Linear, lora_alpha=lora_alpha, r=lora_r)
            else:
                linear_layer = partial(lora.Linear, lora_alpha=lora_alpha, r=lora_r)
        else:
