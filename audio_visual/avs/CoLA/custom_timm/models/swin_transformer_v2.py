...
from custom_timm.layers import PatchEmbed, Mlp, DropPath, to_2tuple, trunc_normal_, _assert, ClassifierHead
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        if use_lora: 
            if lora_fusion:
                self.qkv =  lora.CMMergedLinear(dim, dim * 3, bias=False, r=lora_r, lora_alpha=lora_alpha, enable_lora=[True, True, True], c_features=lora_c_dim, reduction_ratio=lora_reduction, lora_c_scaling=lora_c_scaling)
                # self.qkv =  lora.MergedLinear(dim, dim * 3, bias=False, r=lora_r, lora_alpha=lora_alpha, enable_lora=[True, True, True])
            else:
                self.qkv =  lora.MergedLinear(dim, dim * 3, bias=False, r=lora_r, lora_alpha=lora_alpha, enable_lora=[True, True, True])
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        #old linear forward dont work with LoRA we just solve it by put the bias in the weight and remove after 
        ...
        # print(torch.equal(temp_qkv,qkv))
        # exit()
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerV2Block(
                ...
                lora_r = lora_r,
                lora_alpha = lora_alpha,
                lora_c_dim = lora_c_dim, ## added for LoRA
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            layers += [SwinTransformerV2Stage(
                ...
                lora_r=lora_r,
                lora_alpha=lora_alpha,
                lora_c_dim = lora_c_dim[i],
                lora_fusion_layers = lora_fusion_layers[i],
                lora_reduction = (lora_reduction**4),
                lora_c_scaling=lora_c_scaling,
            )]
            in_dim = out_dim
            if i > 0:
    ...
    def forward(self, x):
        ...
        # commention out for degugging lora support
        # since not using head for AVS
