...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_lora',  default=True, action='store_true', help='Apply LoRA to the model') 
    ...
    parser.add_argument('--output_dir', type=str, help='Gradinet Accumulation')
    parser.add_argument("--audio_length", default=1.95, type=float)
    ...
    with torch.no_grad():
        for n_iter, batch_data in enumerate(test_dataloader):
            ...
            imgs, audio_spec, audio, mask, category_list, video_name_list = batch_data
            imgs = imgs.cuda()
            audio = audio.cuda()
            mask = mask.cuda()
            ...
            output, _, _ = model(imgs, audio_spec) # [5, 1, 224, 224] = [bs=1 * T=5, 1, 224, 224]
