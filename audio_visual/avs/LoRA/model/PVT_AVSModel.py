...
def vis_qkv_attn(attn_module, x, mask=None):
...
class Pred_endecoder(nn.Module):
	...
	def __init__(self, channel=256,opt=None, config=None, vis_dim=[64, 128, 320, 512], tpavi_stages=[], tpavi_vv_flag=False, tpavi_va_flag=True):
		...
		elif opt.swin_size == "swin_b":
			self.x1_linear = nn.Linear(128,64)
			self.x2_linear = nn.Linear(256,128)
			self.x3_linear = nn.Linear(512,320)
			self.x4_linear = nn.Linear(1024,512)
		else:
			raise ValueError("Swin eerro")

		self.audio_linear = nn.Linear(768,128)
		
		if opt.swin_size == "swin_l":
			self.swin_v = model_backbone.get_timm_swinv2_l_pretrained_model(n_classes=1000, imgnet=True, use_lora=opt.use_lora, lora_r=opt.lora_r, lora_alpha=opt.lora_alpha)
		elif opt.swin_size == "swin_b":
			self.swin_v = model_backbone.get_timm_swinv2_b_pretrained_model(n_classes=1000, imgnet=True, use_lora=opt.use_lora, lora_r=opt.lora_r, lora_alpha=opt.lora_alpha)
		else:
			raise ValueError("Swin eerro")
		print("loaded swin")
		# self.audio_a = model_backbone.get_timm_maxast_pretrained_model(n_classes=527, imgnet=False,asPth=AS2M_PATH, use_lora=opt.use_lora, lora_r=opt.lora_r, lora_alpha=opt.lora_alpha)
		self.audio_a = sslam_backbone.get_sslam_backbone(use_lora=opt.use_lora, lora_r=opt.lora_r, lora_alpha=opt.lora_alpha)
		if opt.use_lora:
			print("Using LoRA")
	...
	def forward(self, x, audio_feature=None):
		...
		# print(f"ffa shape: {ffa.shape}, f_a shape: {f_a.shape}")

		# try:
		# 	assert torch.equal(ffv, f_v), "Vision features mismatch"
		# 	print("✅ Vision features match exactly!")
		# except AssertionError as e:
		# 	print("Ffa shape", ffv.shape, "F_a", f_v.shape)
		# 	print(f"❌ Vision features mismatch: {e}")
		# 	raise
		# try:
		# 	assert torch.equal(ffa, f_a), "Audio features mismatch"
		# 	print("✅ Audio features match exactly!")
		# except AssertionError as e:
		# 	print("Ffa shape", ffa.shape, "F_a", f_a.shape)
		# 	print(f"❌ Audio features mismatch: {e}")
		# 	raise
		# exit()
		...
		# print(multi_scale[0].shape, multi_scale[1].shape, multi_scale[2].shape, multi_scale[3].shape)

		# This version of Timm return B,  H, W ,C
