...
class EEVGDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit',
                 ...
                 bert_model='bert-base-uncased', is_segment=False):
        self.images = []
        ...
        print('Loading dataset from: {}'.format(self.dataset_root))
        print("Split root: {}".format(split_root))
    ...
    def pull_item(self, idx):
        ...
        if data_source == 'coco':
            ...
            img_path = osp.join("./ln_data/flickr30k", img_file)
        elif data_source == 'vg':
            img_path = osp.join("./ln_data/visual-genome", img_file)
        else:
            img_path = osp.join(self.im_dir, img_file)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_segment:
            return img, np.array(img_mask), np.array(word_id, dtype=int), np.array(word_mask, dtype=int), \
                np.array(bbox, dtype=np.float32), gt_mask
        return img, np.array(img_mask), np.array(word_id, dtype=int), np.array(word_mask, dtype=int), \
