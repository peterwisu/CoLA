...
def train_one_epoch(args, model: torch.nn.Module, scaler, data_loader: Iterable,
                    ...
                    epoch: int, max_norm: float = 0, alpha: float = 1.0, is_collect=False, accumulation_steps=5):
    model.train()

    print("Gradient accum step: ",accumulation_steps)
    # GPU snapshot at start of this epoch
    print(f"[Epoch {epoch}] GPU usage at start of train_one_epoch:")
    get_gpu_utilization()
    ...
    for batch_idx, batch in enumerate(
        ...
        if batch_idx % 200 == 0:
            print(f"[Epoch {epoch} Batch {batch_idx}] GPU usage mid‐epoch:")
            get_gpu_utilization()
...
def validate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for batch_idx, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        if batch_idx % 80 == 0:
            print(f"[Eval Batch {batch_idx}] GPU usage :")
            get_gpu_utilization()
            # break
