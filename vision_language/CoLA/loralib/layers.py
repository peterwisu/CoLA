...
class CMLinear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0:
            ...
            self.lora_scaling_c = nn.Parameter(self.weight.new_full((1,), fill_value=self.lora_alpha / self.r))
    ...
    def forward(self, x_m: torch.Tensor, x_c: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            ...
            if is_vision:
                cross_result = cross_result.reshape(b,h2,w2,-1) 
            # return F.linear(x_m, T(self.weight), bias=self.bias)
          
            outs = main + cross_result
            return outs


## CoLA merged linear Layer
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0 and any(enable_lora):
            ...
            self.lora_scaling_c = nn.Parameter(self.weight.new_full((sum(enable_lora),),fill_value=self.lora_alpha / self.r))
...
### Hypernetwork MLP for generating phi in CoLA
class Mlp(nn.Module):
