...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str, bb_ckpt_dir: str = None, use_lora: bool = False, use_lora_r: int = 8, use_lora_alpha: int = 8, do_fusion=None,c_features=None):
        assert name in ('SwinT', 'ViTDet', 'SwinT-S')
        if name == 'SwinT':
            raise NotImplementedError("Not suppose to use SwinT")
            ...
            raise NotImplementedError("Not suppose to use SwinT-S")
            backbone = build_SwinT(name)
            ckpt_path = "./checkpoints/swin_small_patch4_window7_224_22k.pth"
            backbone.init_weights(pretrained=ckpt_path)
        elif name == 'ViTDet':
            backbone = build_ViTDet(use_lora, use_lora_r, use_lora_alpha, do_fusion, c_features)
            import pickle as pkl
            model_ckpt_path = bb_ckpt_dir+"model_final_435fa9.pkl"
            print("Loading ViTDet checkpoint from {}".format(model_ckpt_path))
...
def build_backbone(args):
    ...
    # args.vision_fusion = [True] * 6 + [False] * 6
    # print(args.vision_fusion)
    backbone = Backbone(args.backbone, args.bb_ckpt_dir, args.use_lora, args.lora_r, args.lora_alpha, args.vision_fusion, args.vislora_c_feats )
